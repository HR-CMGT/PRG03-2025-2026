...
const apiBaseUrl = 'https://prg03-voorbeeld.test/examples/week8-openai-php/webservice/';
let questionForm;
let questionInput;
let answerParagraph;
...
function speakAnswerClickHandler() {
  if (synth.speaking) {
    ...
    msg.voice = synth.getVoices().find(voice => voice.lang === 'nl-NL');
    synth.speak(msg);
  }
}
...
function showGeneratedImages(data) {
  ...
  for (const imageData of data.data) {
    ...
    //Image based on base64 string from data
    const image = document.createElement('img');
    image.src = `data:image/png;base64,${imageData.b64_json}`;
    image.alt = 'Generated DALL-E image';
    image.loading = 'lazy';
    figure.appendChild(image);

    //Caption with the revised Prompt GPT created based on my input
    const caption = document.createElement('figcaption');
    caption.innerText = imageData.revised_prompt;
